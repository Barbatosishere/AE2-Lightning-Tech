...
吃满保底模型输入容量 C    = 576 copies
吃满保底模型处理周期 P    = 20 tick
吃满保底模型单次处理量 Q  = 9 copies
...
| C | 10,240 |
| D | 2,560 |
| E | 40（纯容量理论值；正式发配基线改用每目标每 100 tick 一次） |
...
| E：576 / 20 tick / 9 copies | 1（保底基线） | 512 | 4 | 2,048 |
...
模型 E 专门验证“小处理量、长处理周期、输入容量略大于已证明批次”的吃满保底路径。容量公式给出的理想值低于每目标每 100 tick 一次，不能作为长期完全不访问目标的理由，因此其正式基线固定为：

```text
fallbackBaselinePerTarget100 = 1 physical push
dispatchMetricE = actualPhysicalPushes100 / N * 100%
dispatchMetricE <= 400%
```

也就是说，“100 tick 一次”是 100% 保底基线；400% 验收允许每个目标在任意滑动 100-tick 窗口内最多四次物理 push，全部 512 个目标最多 2,048 次。实现可以把四次预算分散到窗口内，但不得以超过四次的连续失败探测换吞吐。
...
模型 E 允许某次保底访问因容量不足而零接收，因此不以窗口内 accepted copies 判断公平；改为比较各目标的 `physicalPushes`，要求每个目标在每个正式 100-tick 窗口至少被访问一次，且最多访问目标与最少访问目标之比不超过 2。
...
adaptiveBatch_fillFallback_capacity576_period20_targets512_maxProcess9
adaptiveBatch_random_capacity2048_period2to5_targets512_maxProcess512to1024
```

每个参数组合运行两个测试：

- `convergesAndStaysWithinLimitsFor200Ticks`：验证十倍处理时间学习期后的持续吞吐、发配指标和公平性；若模型的 `startupTicks + 100` 超过 200，则该模型延长到足以包含一个正式验收窗口（E 实际运行 300 tick）。
- `doesNotRegressAcross5000Ticks`：在长时间滑动窗口中重复相同验收，禁止后期节奏漂移。

另加两个有限任务公平性用例：总请求分别为 512 和 2,048 copies、目标数均为 512。前者必须每目标恰好收到 1 copy；后者必须在安全 `1, 1, 2` 冷启动序列内每目标恰好收到 4 copies，不允许只让前一小部分目标完成任务。
...
新增模型 E 后，调度器在较大的已证明批次持续遭到拒绝、且连续 100 tick 没有一次不受请求份数限制的完整接收时，进入吃满保底。当前门槛为已证明批次至少 64 copies 且累计至少 64 次真实拒绝；较小批次继续使用原有学习 cadence，避免普通慢设备被保底机制过量预填。保底仍由公平队列决定访问顺序，但 100-tick 公平计数过期后不得把目标已经证明过的物理 batch 截回 1 copy；保底最多每 25 tick 访问一次，即任意 100-tick 窗口最多四次。连续两次完整接收后退出保底，重新进入普通自适应。
...
| C | 99.80% | 100.00% | 200.00% | 200.00% |
| D | 99.80% | 100.00% | 400.00% | 400.00% |
| E | 99.60% | 100.00% | 400.00% | 400.00% |
| R | 99.50% | 99.04% | 516.12% | 756.71% |

有限任务额外验证 512 copies 与 2,048 copies 两种请求：512 个目标分别恰好获得 1 与 4 copies；2,048-copy 用例只执行安全的 `1, 1, 2` 三轮，不因大任务调度优化集中到少数目标。
